...
def init_config(config_path: Path) -> None:
    ...
    values: Dict[str, str] = {
        ...
        "QUANT_ORDER_SHARES": existing.get("QUANT_ORDER_SHARES", DEFAULT_QUANT_ORDER_SHARES),
        "QUANT_CAPITAL_USDC": existing.get("QUANT_CAPITAL_USDC", "300"),
        "QUANT_MARKET_MAX_USDC": existing.get("QUANT_MARKET_MAX_USDC", DEFAULT_QUANT_MARKET_MAX_USDC),
        "QUANT_MAX_TRADES_PER_MARKET": existing.get("QUANT_MAX_TRADES_PER_MARKET", DEFAULT_QUANT_MAX_TRADES_PER_MARKET),
...
def print_config_summary(config_path: Path, require_private_key: Optional[bool] = None) -> bool:
    ...
    print(
        ...
        f"max_drawdown={config.quant_max_drawdown_usdc} USDC"
    )
...
def update_quant_config(config_path: Path) -> None:
    ...
    values = {
        ...
        "QUANT_ORDER_SHARES": prompt_text("每次量化下单份额 QUANT_ORDER_SHARES", existing.get("QUANT_ORDER_SHARES", DEFAULT_QUANT_ORDER_SHARES)),
        "QUANT_CAPITAL_USDC": prompt_text("量化模拟本金 QUANT_CAPITAL_USDC", existing.get("QUANT_CAPITAL_USDC", "300")),
        "QUANT_MARKET_MAX_USDC": prompt_text("单市场最大模拟成本 QUANT_MARKET_MAX_USDC", existing.get("QUANT_MARKET_MAX_USDC", DEFAULT_QUANT_MARKET_MAX_USDC)),
        "QUANT_MAX_TRADES_PER_MARKET": prompt_text("单市场最多模拟笔数 QUANT_MAX_TRADES_PER_MARKET", existing.get("QUANT_MAX_TRADES_PER_MARKET", DEFAULT_QUANT_MAX_TRADES_PER_MARKET)),
...
def configured_quant_state_file(config_path: Path) -> Path:
    return load_config(config_path).quant_state_file
...
def clear_quant_data(config_path: Path) -> None:
    ...
    if state_file.exists():
        state_file.unlink()
        print(f"[OK] 已清空 AI量化状态: {state_file}")
    else:
        print(f"[INFO] AI量化状态不存在: {state_file}")
    ...
    if args.command == "menu":
        ...
        if not any(
            ...
            set_env_value(
                config_path,
                "QUANT_ORDER_SHARES",
                args.order_shares or existing.get("QUANT_ORDER_SHARES", DEFAULT_QUANT_ORDER_SHARES),
            )
            ...
            set_env_value(
                ...
                args.market_max_usdc or existing.get("QUANT_MARKET_MAX_USDC", DEFAULT_QUANT_MARKET_MAX_USDC),
            )
            set_env_value(
                config_path,
                "QUANT_MAX_TRADES_PER_MARKET",
                args.max_trades_per_market
                or existing.get("QUANT_MAX_TRADES_PER_MARKET", DEFAULT_QUANT_MAX_TRADES_PER_MARKET),
            )
            set_env_value(
                config_path,
                "QUANT_REBUY_COOLDOWN_SEC",
                args.rebuy_cooldown_sec or existing.get("QUANT_REBUY_COOLDOWN_SEC", DEFAULT_QUANT_REBUY_COOLDOWN_SEC),
            )
            set_env_value(
                config_path,
            ...
            set_env_value(
                ...
                args.max_drawdown_usdc or existing.get("QUANT_MAX_DRAWDOWN_USDC", DEFAULT_QUANT_MAX_DRAWDOWN_USDC),
            )
